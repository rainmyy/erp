...
<dl id="customer" class="tabs">
	...
	<dt title="与客户相关的客户服务信息"{$service['off']}>客服{$service['count']}</dt>
	...
	function insert(){
		if($this->kclass->input['requestMethod'] != 'post'){
			...
			if($this->kclass->input['email'] == ''){
				$e .= '<li>请填写客户的 邮箱。</li>';
			}elseif(strlen($this->kclass->input['email'])>100){
...
EOF;
		$this->kclass->page['onload'] .= 'dc.tabhover();dc.tabhover();var dates=$(\'#startDate,#endDate\').datepicker({onSelect:function(selectedDate){var option=this.id==\'startDate\'?\'minDate\':\'maxDate\',instance=$(this).data(\'datepicker\'),date=$.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat,selectedDate,instance.settings);dates.not(this).datepicker(\'option\',option,date);}});';
	...
	function mmlist(){
		...
		if($this->kclass->input['customersourceid']>0){
			$condition .= " AND `customer`.customersourceid='".$this->kclass->input['customersourceid']."'";
...
EOF;
	...
	function insertQuoting(){
		...
		$this->kclass->DB->query("
			INSERT INTO `quoting` (`customerid`, `inquiryid`, `contact`, `currencyid`, `portid`, `deliverytermid`, `paymenttermid`, `shipmethodid`, `dateline`,`standard`, `material`, `technics`, `remark`, `modified`, `creator`, `created`) 
			VALUES ('".$this->kclass->input['customerid']."', '".$this->kclass->input['inquiryid']."', '".$this->kclass->input['contact']."', '".$this->kclass->input['currencyid']."', '".$this->kclass->input['portid']."', '".$this->kclass->input['deliverytermid']."', '".$this->kclass->input['paymenttermid']."', '".$this->kclass->input['shipmethodid']."','{$dateline}', '".$this->kclass->input['standard']."', '".$this->kclass->input['material']."', '".$this->kclass->input['technics']."', '".$this->kclass->input['remark']."', '".TIMENOW."', '".$this->kclass->user['userid']."', '".TIMENOW."')
		");
		
		
		}else{
			$this->kclass->DB->query("
			INSERT INTO `quoting` (`customerid`,`typeid`, `contact`, `currencyid`, `portid`, `deliverytermid`, `paymenttermid`, `shipmethodid`, `dateline`,`standard`, `material`, `technics`, `remark`, `modified`, `creator`, `created`) 
			VALUES ('".$this->kclass->input['customerid']."','".$this->kclass->input['typeid']."','".$this->kclass->input['contact']."', '".$this->kclass->input['currencyid']."', '".$this->kclass->input['portid']."', '".$this->kclass->input['deliverytermid']."', '".$this->kclass->input['paymenttermid']."', '".$this->kclass->input['shipmethodid']."','{$dateline}', '".$this->kclass->input['standard']."', '".$this->kclass->input['material']."', '".$this->kclass->input['technics']."', '".$this->kclass->input['remark']."', '".TIMENOW."', '".$this->kclass->user['userid']."', '".TIMENOW."')
		");
		}
...
<dl id="customer" class="tabs">
    ...
    <table class="hundred">
        ...
        <td>支持交货条款：</td><td>{$deliveryterm}</td>
		</tr>
      <tr class="odd">
...
function STQuotingview(){
   ...
   $item=$this->kclass->DB->queryFirst("
	...
	$body.=<<<EOF
		...
		<tr class="odd"><td width="200px">支持交货条款</td><td>{$item['detitle']}({$item['deentitle']})</td><td	width="200px">支持地区</td><td>{$item['ptitle']}({$item['pentitle']})</td></tr>
...
function addSTQuoting(){	
	...
	$this->kclass->DB->query("INSERT INTO `stquoting`(`typeid`,`deliverytermid`,`cntitle`,`portid`,`entitle`,`currencyid`,`standrad`,`material`,`technics`,`remark`,`modified`,`creator`,`created`) 
	VALUES('".$this->kclass->input['typeid']."','".$this->kclass->input['deliverytermid']."','".$this->kclass->input['cntitle']."','".$this->kclass->input['portid']."','".$this->kclass->input['entitle']."','".$this->kclass->input['currencyid']."','".$this->kclass->input['standard']."','".$this->kclass->input['material']."','".$this->kclass->input['technics']."','".$this->kclass->input['remark']."','".TIMENOW."','".$this->kclass->user[ 'userid' ]."','".TIMENOW."')"
	);
	$quotingid=$this->kclass->DB->insertID();
	for($k=1;$k<=$j;$k++){
		$this->kclass->input['itemQuantity'][$k]=intval($this->kclass->input['itemQuantity'][$k]);
...
EOF;
				...
				$item=$this->kclass->DB->queryFirst("
					...
					st.cntitle AS stcntitle,st.entitle AS stentitle,st.created AS stcreated,de.title AS  detitle,de.entitle AS deentitle,st.*
					FROM `stquoting` AS st
					LEFT JOIN `port` AS p ON ( p.portid=st.portid)
					LEFT JOIN `currency` AS c ON (c.currencyid=st.currencyid)
					LEFT JOIN `quotingtype` AS qt ON (qt.typeid=st.typeid)
  ...
  // 
	...
	function sendmail(){
		...
		if(isset($e)){
			$this->kclass->messager(array(
				'title' => '将报价单以邮件回复给客户',
				'text' => '您在给客户发送邮件的过程中有以下错误：<ul>'.$e.'</ul>',
				'url' => 'javascript:history.back()',
				'sec' => 3
			));
		}
		$subject="QUOTING FOR CUSTOMER";
		$message="DEAR".$linkman."
		...
		$this->kclass->mailers(array('from'=>''.$this->kclass->user['email'].'','fromname'=>''.$this->kclass->user['username'].'','smtphost'=>'mail.n-wisdom.hk','smtphelo'=>'n-wisdom.hk','smtpusername'=>''.$this->kclass->user['email'].'','smtppassword'=>'','address'=>''.$to.'','to'=>''.$to.'','cc'=>''.$cc.'','bcc'=>'','subject'=>''.$subject.'','message'=>''.$message.''));
		$this->kclass->messager(array(
			'title' => '将报价单以邮件回复给客户',
			'text' => '给客户的邮件已发送成功!',
			'url' => '/s.php?module=customer&action=view&customerid='.$this->kclass->input['customerid'],
