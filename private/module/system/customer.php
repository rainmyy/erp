...
<tr class="even">
    <td>且联系人备注包括:</td>
	<td><input type="text" style="width:244px" name="remark" value=""><span class="small gray">匹配竹联系人或次联系人的备注</span></td>
</tr>
<tr class="odd">
	<td>且 商业类型 是：</td>
	<td>{$business}</td>
...
EOF;
	...
	function mmlist_test(){
		...
		$lowNum =$middleNum=$highNum=$allNum= array();
	 $order = $this->kclass->DB->query("
					SELECT `order`.customerid AS id 
					FROM `order`
					WHERE type='PI'
				");
				$orderNum=array();
   ...
   $num=array_count_values($orderNum);
   //print_r($num);
   foreach($num as $key=>$val){
	  if($val<$customerType['max']&&$val>=$customerType['min']){
		   $middleNum[]=$key;
	   }elseif($val>=$cusstomerType['max']){
		    $highNum[]=$key;
	   }
   }
  $lowNum=array_diff($allNum,$highNum,$middleNum);
	$customerTitle = array(
				'high'=>'常客(超过3个订单)',
	...
	$body .= '
		<dl id="customer"  class="tabs" style="display: block;">
		...
		';
		//$body.='<dd>111</dd><dd>222</dd><dd>333</dd><dd>444</dd>';
		...
		foreach($customerTitle as $k=>$v){
			...
			$body .='<table class="hundred mytable tablesorter"><thead><tr><th width="26">ID</th><th width="35">订单</th><th width="100">审核状态</th><th>客户名称</th><th width="40">来源</th><th width="60">来源时间</th><th>联系人</th><th>邮箱</th><th width="150">电话</th><th >地区</th><th width="50">销售</th><th width="90">创建</th><th width="90">修改</th><th width="26">选项</th></tr></thead><tbody>' ;
              $customerImplode=array();
			
            if($k=='high'){
				$customerImplode=$highNum;
			}elseif($k=='middle'){
               $customerImplode=$middleNum;
			}elseif($k=='low'){
				$customerImplode=$lowNum;
			}elseif($k=='all'){
				$customerImplode=$allNum;	
			}
                $sql = "
					...
					LEFT JOIN `user` AS c ON (c.userid=`customer`.creator) 
					LEFT JOIN `user` AS t ON (t.userid=`customer`.tracker) 
					WHERE  `customer`.customerid IN(".implode(',',$customerImplode).")
					";
			...
			$body .= $this->kclass->input['layout']=='</tbody></table>';
			}
			  $body .= "</dl>
		<script type=\"text/javascript\">
...
' , 'body'=>$body));
	...
	function mmlist(){
		...
		if($this->kclass->input['remark']!=''){
			$condition.=" AND (INSTR(LCASE(`customer`.remark),'".(strtolower($this->kclass->input['remark']))."')>0)";
		}
	...
	//echo json_encode($this->kclass->input['quotingid']);exit;
       ...
       /*  if($_POST['verify'] != 1){
        ...
        if($this->kclass->input['customerid'])
        {
			...
			$mail->From="service@wisdom.hk";
			$mail->Username="service@wisdom.hk";
			$mail->Password="m6ntc9xc";
