	...
	function view(){
		...
		if($outbound){
			...
			$items=$this->kclass->DB->query("
				...
				LEFT JOIN `unit` ON (unit.unitid=poi.unitid)
			...
			if($this->kclass->DB->numRows()){
				...
				while($item=$this->kclass->DB->fetchArray($items)){
					...
					//总数若是小数则保留小数点后两位
					if(is_float($totalCount[$item['versionid']])){
						$totalCount[$item['versionid']] = number_format($totalCount[$item['versionid']],2,".","");
					}
					$store=0;
...
EOF;
				$this->kclass->page['title'].='新建出库单';
				$this->kclass->page['main']=$this->kclass->skin->dbody(array('title'=>$this->baseurl.=' - <a href="/s.php?module=outbound&action=list">出库列表</a> - 新建出库单', 'right'=>'<a href="/s.php?module=outbound&action=list">返回列表</a>','body'=>$body));
			}elseif($this->kclass->input['typeid']==14 AND $this->kclass->input['entrustid']<=0){
				$omorder=$this->kclass->chooserOmorders(array('name'=>'entrustid','hasBlank'=>1,'topname'=>'请选择','width'=>350));
				$body=<<<EOF
<form action="/s.php?module=outbound&action=add" method="get">
<input type="hidden" name="module" value="outbound">
<input type="hidden" name="action" value="add">
...
<thead>
<tr>
<th colspan="4">新建出库申请：请选择外发加工订单</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td title="">请选择外发加工订单：<span class="red bold">*</span></td>
<td>{$omorder}</td>
</tr>
<tr class="odd" nohover>
<td colspan="4" class="center" nohover>
...
EOF;
				...
				if($this->kclass->input['workcenterid']>0){ // 从生产任务中下单
					...
					}elseif($typeid==19 OR $typeid==20){//订单样品单生产任务出库
						if($typeid==19){
							$module='order';
							$mid=array_unique(array_filter($this->kclass->input['orderid']));
							$url='';
							foreach($mid as $val){
							$order=$this->kclass->DB->queryFirst("SELECT orderid,orderno FROM `order` WHERE killed=0 AND orderid='".$val."'");
							$url.='<a href="/s.php?module=order&action=view&orderid='.$val.'&type=PO">'.$order['orderno'].'</a>　';
							}
							$r='<tr class="odd"><td>订单编号：</td><td>'.$url.'</td><td></td><td></td></tr>';
							
						}else{
							$module='sample';
							$mid=array_unique(array_filter($this->kclass->input['sampleid']));
							foreach($mid as $val){
							$sample=$this->kclass->DB->queryFirst("SELECT sampleid,sampleno FROM `sample` WHERE killed=0 AND sampleid='".$val."'");
							$url.='<a href="/s.php?module=sample&action=view&sampleid='.$val.'">'.$sample['sampleno'].'</a>　';
							}
							$r='<tr class="odd"><td>样品单编号：</td><td>'.$url.'</td><td></td><td></td></tr>';
							
						}
					$workcenter=$this->kclass->DB->queryFirst("SELECT title FROM workcenter WHERE workcenterid='".$this->kclass->input['workcenterid']."'");
						$itemList='<tr class="center even" nohover><td width="30">ID</td><td width="80">物资编号</td><td width="150">物资详情 <span class="bold red">*</span></td><td>物资规格<span class="bold red">*</span></td><td width="100">版本</td><td width="150">需求数量 <span class="red bold">*</span></td><td width="150">出库时间 <span class="red bold">*</span></td><td width="200">备注</td><td width="30">选择</td></tr>';
						$materialitem=$this->kclass->DB->query("
									SELECT 
									ri.module,ri.mid,ri.mitemid,ri.materialid,ri.versionid,ri.materialitemid
									...
									LEFT JOIN taskitem AS ti ON (ti.requirementitemid=ri.itemid)
									LEFT JOIN revise AS r ON(r.reviseid=ri.reviseid)
									WHERE ri.module='".$module."' AND ri.mid IN(".implode(',',$mid).") AND ti.workcenterid='".$this->kclass->input['workcenterid']."' AND 
									ti.ifChooser=0
						");
						if($num=$this->kclass->DB->numRows()){
							$array=array();
							$i=1;
							while($materialitems=$this->kclass->DB->fetchArray($materialitem)){
								$array[$i]=$materialitems['materialid'].','.$materialitems['materialitemid'];
								$i++;
							}
						}	
						$array=array_filter(array_unique($array));
						if($num>0){
							$array=$this->kclass->sortArray($array);
							$getmaterial=$this->_getMaterial(array('module'=>$module,'num'=>$num,'array'=>$array,'mid'=>$mid,'workcenterid'=>$this->kclass->input['workcenterid']));
							$itemList.=$getmaterial['panel'];
							$id.=$getmaterial['id'];
						
						}else{
							$itemList.='<tr class="small"><td colspan="9">生产组<span class="bold">'.$workcenter['title'].'</span>没有该订单的详细信息</td></tr>';
						}
						/**旧的列表
						$item=$this->kclass->DB->query("
									SELECT ri.module,ri.mid,ri.mitemid,ri.materialid,ri.versionid,ri.materialitemid,ri.statusid,ri.reviseid,ri.dateline,ri.quantity,ri.remark,ri.requirementid,
									...
									ti.taskid,ti.workcenterid,ti.requirementitemid
								FROM requirementitem AS ri
								LEFT JOIN material AS m ON (m.materialid=ri.materialid)
								LEFT JOIN materialitem AS mi ON (mi.itemid=ri.materialitemid)
								...
								LEFT JOIN taskitem AS ti ON (ti.requirementitemid=ri.itemid)
								LEFT JOIN revise AS r ON (r.reviseid=ri.reviseid)
									WHERE ri.module='".$module."' AND ri.mid IN (".implode(',',$mid).")  AND ti.workcenterid='".$this->kclass->input['workcenterid']."' AND ti.ifChooser=0
						");
						...
						if($num=$this->kclass->DB->numRows()){
							...
							while($items=$this->kclass->DB->fetchArray($item)){
							   $array[$i]=$items;
							  
								//判断是否存在子BOM
								if($items['originid']==1){
									$child=$this->kclass->bomChildList(array('parentid'=>$items['materialid'],'materialitemid'=>$items['materialitemid'],'versionid'=>$items['versionid'],'loopNum'=>1,'getCount'=>1));
									$ifbom=$this->kclass->iif($child==1,'是','否');
								}else{
									$ifbom='-';
								}
								//$task=$this->kclass->DB->queryFirst("SELECT ti.workcenterid FROM taskitem WHERE ti.itemid='".$items['']."'");
								// 需求计划明细
				     $materialInfo=$this->kclass->getMaterial(array('materialid'=>$items['materialid'],'itemid'=>$items['materialitemid'])); // 获得物资信息
					 $child=$this->kclass->bomChildList(array('parentid'=>$items['materialid'],'materialitemid'=>$items['materialitemid'],'versionid'=>$items['versionid'],'quantity'=>$items['quantity'],'loopNum'=>1,'ifGeneral'=>0));
								$itemList.='<tr class="'.$this->kclass->iif($this->kclass->rotate(),'odd','even').'  bold bgGreen">
												<td><input type="hidden" name="materialid['.$i.']" value="'.$items['materialid'].'"/>
												<input type="hidden" name="materialitemid['.$i.']" value="'.$items['materialitemid'].'">
												<input type="hidden" name="taskItemid['.$i.']" value="'.$items['taskid'].'">
												<input type="hidden" name="versionItemid['.$i.']" value="'.$items['versionid'].'">
												<input type="hidden" name="workcenterItem['.$i.']" value="'.$items['workcenterid'].'">'.$i.'</td>
												<td><a href="'.$materialInfo['url'].'">'.$materialInfo['no'].'</a></td>
												<td>'.$materialInfo['material'].'</td>
												<td>'.$materialInfo['standard'].'</td>
												<td>'.$this->kclass->iif($items['change']=='',$items['version'],'<a class="tip" href="#" title="( '.$items['change'].' )">'.$items['version'].$this->kclass->iif($items['itemno']!='',' ('.$items['itemno'].') ','').'</a>').'</td>
												<td><input type="hidden" name="itemQuantity['.$i.']" value="'.$items['quantity'].'">'.$items['quantity'].'</td>
												<td><input type="text" id="outTime'.$i.'" name="itemOutTime['.$i.']" size="10" value="'.date('Y-m-d',TIMENOW).'" ></td>
												<td><input type="text" name="remarkitem['.$i.']" value="'.$items['remark'].'"></td>
												<td align="center"><input type="checkbox" name="selectItemid['.$i.']" value="'.$items['materialid'].'"   ></td>
												</tr>';
							                   $id.=$this->kclass->iif($i<=$num,'#outTime'.$i.',','#outTime'.$i);
							//print_r($child);
											if(!empty($child['bom'])){
										...
										$itemList.='<tr class="'.$this->kclass->iif($this->kclass->rotate(),'odd','even').' small		'.$this->kclass->iif($val['ifGeneral']==0,$this->kclass->iif($val['quantity']>$version['qualified'],'bgYellow',''),'').'">
										<td><input type="hidden" name="bomItemid['.$k.']" value="'.$val['materialid'].'" >
										<input type="hidden" name="bomMaterialitemid['.$k.']" value="'.$val['materialitemid'].'">
										<input type="hidden" name="bomTaskitemid['.$k.']" value="'.$items['taskid'].'">'.$k.'</td>
										<td><a href="'.$val['url'].'">'.$val['no'].'</a></td>
										<td>'.$val['title'].'</td>
										<td>'.$val['standard'].'</td>
										<td><input type="hidden" name="bomVersionid['.$k.']" value="'.$val['versionid'].'">'.$val['version'].'</td>
										<td><input type="text" name="quantity['.$k.']" value="'.$val['quantity'].'"></td>
										<td><input type="text" id="bomoutTime'.$k.'" name="bomOutTime['.$k.']" size="10" value="'.date('Y-m-d',TIMENOW).'"></td>
										<td><input type="text" name="bomRemark['.$k.']" value="'.$val['remark'].'"></td>
										<td align="center"><input type="checkbox" name="selectBomid['.$k.']" value="'.$val['materialid'].'"></td>
										</tr>';
										   // $id.=$this->kclass->iif($k<30,'#bomoutTime'.$k.',','#bomoutTime'.$k);
										   $id.='#bomoutTime'.$k.',';
												$k++;	
					                       }  
										}		
					...
					}elseif($this->kclass->input['typeid']==14 AND $this->kclass->input['entrustid']>0){ //外发加工单出库
							//echo $this->kclass->input['typeid'];
							if($this->kclass->input['entrustid']>0){
								$entrust=$this->kclass->DB->queryFirst("SELECT entrustid,entrustno FROM  `entrust` WHERE killed=0 AND entrustid='".$this->kclass->input['entrustid']."'");
								$r='<tr class="odd"><td>外发加工单编号：</td><td><a href="/s.php?module=entrust&action=view&entrustid='.$this->kclass->input['entrustid'].'">'.$entrust['entrustno'].'</a></td><td></td><td></td></tr>';
								//$module='porder';$mid=$porder['porderid'];
							}
							$itemList='<tr class="center even" nohover><td width="30">ID</td><td>选择物资 <span class="bold red">*</span></td><td width="50">数量 <span class="red bold">*</span></td><td width="50">出库时间 <span class="red bold">*</span></td><td width="100">备注</td></tr>';
							$items=$this->kclass->DB->query("
								SELECT i.itemid,i.materialid,i.versionid,i.quantity,i.requirement,i.remark,
								m.materialno,m.title AS material,m.standard		
								FROM `entrustitem` AS i
								LEFT JOIN `material` AS m ON (m.materialid=i.materialid)
								WHERE entrustid='{$this->kclass->input['entrustid']}' AND i.killed=0
								ORDER BY i.itemid ASC
							");
							if($this->kclass->DB->numRows()){
								$i=1;
								while($item=$this->kclass->DB->fetchArray($items)){
									$formermaterial=$this->kclass->DB->queryFirst("SELECT materialid FROM material WHERE entrustMaterialid='".$item['materialid']."'");
									$material=$this->kclass->DB->queryFirst("SELECT *,mv.versionid
									FROM material AS m
									LEFT JOIN materialversion AS mv ON (m.materialid=mv.materialid)
									WHERE m.killed=0 AND m.materialid='".$item['materialid']."'");
									$itemList.='<tr class="'.$this->kclass->iif($this->kclass->rotate(), 'odd', 'even').' small">
									<td><input type="hidden" name="itemId['.$i.']" value="'.$i.'" />'.$i.'</td>									
									<td>
										<input type="hidden" name="itemMaterialid['.$i.']" value="'.$material['materialid'].'" />
										<input type="hidden" name="itemVersionid['.$i.']" value="'.$material['versionid'].'" />
										<a href="/s.php?module=material&action=view&materialid='.$material['materialid'].'">'.$material['materialno'].'</a>　'.$material['material'].' '.$material['standard'].'</td>
									<td><input type="text" name="itemQuantity['.$i.']" size="10" value="'.$item['quantity'].'"></td>
									<td><input type="text" id="outTime'.$i.'" name="itemOutTime['.$i.']" size="10" value="'.date('Y-m-d',TIMENOW).'"></td>
									<td><input type="text" name="itemRemark['.$i.']" style="width:150px"></td>
									</tr>';
...
<tr class="even">
	<td class="small gray" colspan="11">
		...
		一张单最多只能填入10条数据<br/>　　　　　 外发加工类型的出库单物资明细中的物资是外发加工前的物资编号，加工完成后入库时则是外发加工后的物资编号
		</td>
</tr>
</tbody>
</table>
...
EOF;
	...
	function insert(){
		...
		if($this->kclass->input['requestMethod'] != 'post'){
			...
			if($this->kclass->input['workcenterid']>0){
				...
				if($this->kclass->input['typeid']==15 OR $this->kclass->input['typeid']==16){
					...
					for($i=1;$i<=$j;$i++){
                     if($this->kclass->input['itemOutTime'][$i]!=''){
						 $hasItem=1;
					 }
					}
					unset($i);
					$k=count($this->kclass->input['bomItemid']);
					for($i=1;$i<=$k;$i++){
						if($this->kclass->input['itemMaterialid'][$i]>0 AND $this->kclass->input['itemQuantity'][$i]>0 AND $this->kclass->input['bomOutTime'][$i]!='' ){
							$hasBom=1;
						}
					}
					unset($i);
				...
				for($i=1;$i<=$j;$i++){
					if(($this->kclass->input['itemMaterialid'][$i]>0 OR $this->kclass->input['productId'][$i]>0 or $this->kclass->input['purchaseId'][$i]>0) AND $this->kclass->input['itemQuantity'][$i]!='' AND $this->kclass->input['itemOutTime'][$i]!=''){
						//echo $this->kclass->input['typeid'];
						if($this->kclass->input['typeid']==15 OR $this->kclass->input['typeid']==16 OR $this->kclass->input['typeid']==14){
							$hasItem=1;
						}else{
		...
		$this->kclass->DB->query("
			INSERT INTO outbound (outboundno,porderid,orderid,sampleid,workcenterid,realtime,oldoutno,typeid,noteTypeid,departmentid,supplierid,applicant,purpose,remark,created,creator)
			VALUES ('".$outboundno."','".$this->kclass->input['porderid']."','".$this->kclass->input['orderid']."','".$sampleid."','".$workcenterid."','".$realtime."','".$this->kclass->input['oldoutno']."','".$this->kclass->input['typeid']."','".$this->kclass->input['noteTypeid']."','".$this->kclass->user['departmentid']."','".$this->kclass->input['supplierid']."','".$this->kclass->input['applicant']."','".$this->kclass->input['purpose']."','".$this->kclass->input['remark']."','".TIMENOW."','".$this->kclass->user['userid']."')
		");
		$outboundid=$this->kclass->DB->insertID();
		if($this->kclass->input['workcenterid']>0){// 任务单出库
			
        if($this->kclass->input['typeid']==15 OR $this->kclass->input['typeid']==16){//按订单出配件不加入损耗
			//插入物资的子BOM
			$k=count($this->kclass->input['selectBomid']);
			for($m=1;$m<=$k;$m++){
				if($this->kclass->input['bomItemid'][$m]>0 AND $this->kclass->input['bomVersionid'][$m]>0 AND $this->kclass->input['bomOutTime'][$m]!=''){
					$bomsd=explode('-',$this->kclass->input['bomOutTime'][$m]);
					$bomoutTime=mktime(0,0,0,$sd[1],$sd[2],$sd[0]);
					$bommaterialid=$this->kclass->iif($this->kclass->input['bomItemid'][$m]>0,$this->kclass->input['bomItemid'][$m],0);
                     $bommaterialitemid=$this->kclass->iif($this->kclass->input['bomMaterialitemid'][$m]>0,$this->kclass->input['bomMaterialitemid'][$m],0);
					 $bomversionid=$this->kclass->iif($this->kclass->input['bomVersionid'][$m]>0,$this->kclass->input['bomVersionid'][$m],0);
					$quantity=intval($this->kclass->input['quantity'][$m]);
					if($bommaterialid>0){
						$material=$this->kclass->DB->queryFirst("
							SELECT materialid,unitid,quantity,materialno
							FROM material 
							WHERE materialid='".$bommaterialid."'
							LIMIT 0,1
						");
					}
					$this->kclass->DB->query("
									...
									VALUES ('outbound','".$outboundid."','".$bommaterialid."','".$bomversionid."','".$bommaterialitemid."',1,'".$this->kclass->input['bomTaskitemid'][$m]."','".$this->kclass->input['quantity'][$m]."','".$material['unitid']."','".$bomoutTime."','".$this->kclass->input['bomRemark'][$m]."','".TIMENOW."','".$this->kclass->user['userid']."')
								");
				}
			}
			//插入非BOM的物资
			$j=count($this->kclass->input['selectItemid']);
			for($i=1;$i<=$j;$i++){
				if($this->kclass->input['materialid'][$i]>0 AND $this->kclass->input['versionItemid'][$i]>0 AND $this->kclass->input['itemOutTime'][$i]!=''){
					$sd=explode('-',$this->kclass->input['itemOutTime'][$i]);
					$outTime=mktime(0,0,0,$sd[1],$sd[2],$sd[0]);
					$materialid=$this->kclass->iif($this->kclass->input['materialid'][$i]>0,$this->kclass->input['materialid'][$i],0);
					$materialitemid=$this->kclass->iif($this->kclass->input['materialitemid'][$i]>0,$this->kclass->input['materialitemid'][$i],0);
					$versionid=$this->kclass->iif($this->kclass->input['versionItemid'][$i]>0,$this->kclass->input['versionItemid'][$i],0);
            ...
            // $child=$this->kclass->bomChildList(array('parentid'=>$materialid,'materialitemid'=>$materialitemid,'versionid'=>$versionid,'quantity'=>$quantity,'loopNum'=>1));
				 $this->kclass->DB->query("
									INSERT preoutbounditem (module,mid,materialid,versionid,materialitemid,ifQualified,taskitemid,quantity,unitid,outTime,remark,created,creator)
									VALUES ('outbound','".$outboundid."','".$materialid."','".$versionid."','".$materialitemid."',1,'".$this->kclass->input['taskItemid'][$i]."','".$this->kclass->input['itemQuantity'][$i]."','".$material['unitid']."','".$outTime."','".$this->kclass->input['itemRemark'][$i]."','".TIMENOW."','".$this->kclass->user['userid']."')
								");					
			...
			//echo $this->kclass->input['porderid'];
			$j=count($this->kclass->input['itemId']);
			for($i=1;$i<=$j;$i++){
				if(($this->kclass->input['itemMaterialid'][$i]>0 OR $this->kclass->input['productId'][$i]>0 OR $this->kclass->input['purchaseId'][$i]>0) AND $this->kclass->input['itemQuantity'][$i]>0 AND $this->kclass->input['itemOutTime'][$i]!=''){
...
EOF;
	...
	//
	function out(){
		$outbound=$this->kclass->DB->queryFirst("SELECT outboundid,outboundno,orderid,sampleid,supplierid,typeid,workcenterid FROM outbound WHERE killed=0 AND ifComplete<>2 AND outboundid='".$this->kclass->input['outboundid']."' LIMIT 0,1");
		if(!$outbound){
			$e='<li>数据错误，很抱歉~</li>';
		}else{
			$item=$this->kclass->DB->queryFirst("
			...
			if($outbound['typeid']!=15 AND $outbound['typeid']!=16){
				...
				if($outbound['workcenterid']==0){
					if($outbound['orderid']>0){
						$inbound=$this->kclass->DB->queryFirst("SELECT inboundid,ifComplete FROM inbound WHERE killed=0 AND orderid='".$outbound['orderid']."' ORDER BY created DESC LIMIT 0,1");
		...
		if($outbound['typeid']==15){ //订单出库
			if($outbound['workcenterid']>0){
				...
				$material='<a href="'.$materialInfo['url'].'">'.$materialInfo['no'].'</a>　'.$materialInfo['material'].'　'.$materialInfo['standard'].'　　版本：'.$item['version'];
			     ...
			     if($item['ifQualified']==1){
				   ...
				   $inItems=$this->kclass->DB->query("
					SELECT ii.quantity,ii.barcodeid,ii.iftransfer,b.barcode,b.arrivalTime,b.qualified AS store
					FROM inbounditem AS ii
					LEFT JOIN barcode AS b ON (ii.barcodeid=b.barcodeid)
					LEFT JOIN preinbounditem AS pi ON (ii.preitemid=pi.itemid) 
					LEFT JOIN inbound AS i ON (i.inboundid=ii.inboundid)
					WHERE (i.killed=0 AND pi.killed=0 AND pi.module='inbound' AND i.ifVerify=1 AND  ii.orderid=".$orderid."  AND ii.materialid='".$item['materialid']."' AND ii.materialitemid='".$item['materialitemid']."') OR (i.killed=0 AND pi.killed=0 AND pi.module='inbound' AND i.ifVerify=1 AND  ii.orderid=".$outbound['orderid']." AND pi.productid='".$item['productid']."' AND  pi.productitemid='".$item['productitemid']."')
				...
				if($this->kclass->DB->numRows()){
					while($inItem=$this->kclass->DB->fetchArray($inItems)){
						$barcodeid = $inItem['barcodeid'];
						$barcode = $inItem['barcode'];
						$arrivalTime = $inItem['arrivalTime'];
						//$outquantity = $inItem['quantity'];
						$store=$inItem['store'];
						if($inItem['iftransfer']!=1){
							$qname = "库存：";
						}else{
							$qname = "借调：";							
						}
						$barcodeSelect.='<option value="'.$barcodeid.'" selected>'.$barcode.' ( 批次时间：'.date('Y-m-d',$arrivalTime).' ) '.$qname.$store.'</option>';	
					}			
				}
			...
			if($outbound['workcenterid']>0){
				...
				$sampleid = $outbound['sampleid'];
				$store=0;
				$preInitems=$this->kclass->DB->query("
					SELECT pi.itemid,ii.iftransfer
					FROM preinbounditem AS pi
					LEFT JOIN inbound AS i ON (pi.mid=i.inboundid)
					LEFT JOIN inbounditem AS ii ON (ii.inboundid= i.inboundid)
					WHERE i.killed=0 AND pi.killed=0 AND pi.module='inbound' AND i.ifVerify=1 AND i.sampleid='".$sampleid."' AND productid='".$item['productid']."' AND productitemid='".$item['productitemid']."'
				...
				if($this->kclass->DB->numRows()){
					while($preInitem=$this->kclass->DB->fetchArray($preInitems)){//						
						$inItem=$this->kclass->DB->queryFirst("
							SELECT ii.quantity,ii.barcodeid,b.barcode,b.arrivalTime,b.qualified AS store
							FROM inbounditem AS ii
							LEFT JOIN barcode AS b ON (ii.barcodeid=b.barcodeid)
						...
						//$outquantity = $inItem['quantity'];
						$store = $inItem['store'];
						if($preInitem['iftransfer']!=1){
							$qname = "库存：";
						}else{
							$qname = "借调：";	
						}
						$barcodeSelect.='<option value="'.$barcodeid.'" selected>'.$barcode.' ( 批次时间：'.date('Y-m-d',$arrivalTime).' ) '.$qname.$outquantity.'</option>';
					}
				}
				$barcodeSelect.='</select>';
				$product=$this->kclass->getProduct(array('productid'=>$item['productid'],'itemid'=>$item['productitemid']));
		...
		if($this->kclass->DB->numRows()){
			...
			while($item=$this->kclass->DB->fetchArray($items)){
				if(!in_array($item['itemid'],$id)){
					...
					if($outbound['typeid']==15){ //订单出库
							if($outbound['workcenterid']>0){//任务单出库
							...
							$barcodeSelect=$this->kclass->chooserBarcode(array('name'=>'barcodeid['.$i.']','condition'=>'materialid='.$item['materialid'],'versionid'=>$item['versionid'],'width'=>250,'ifQualified'=>$item['ifQualified'],'ifMinus'=>1,'hasBlank'=>1));
						}else{//非任务单出库
							$store=0;
							/*$preInitems=$this->kclass->DB->query("
								SELECT pi.itemid
							...
							$inItems=$this->kclass->DB->query("
								...
								LEFT JOIN preinbounditem AS pi ON (ii.preitemid=pi.itemid) 
								LEFT JOIN inbound AS i ON (i.inboundid=ii.inboundid)
								WHERE ii.killed=0 AND pi.killed=0 AND pi.module='inbound' AND i.ifVerify=1 AND ii.orderid=".$outbound['orderid']."  AND pi.productid='".$item['productid']."' AND pi.productitemid='".$item['productitemid']."'
								ORDER BY ii.created DESC
							");
							$barcodeSelect.='<select id="barcodeid'.$i.'" name="barcodeid['.$i.']" style="width:550px"><option value=0></option>';
							if($this->kclass->DB->numRows()){
								while($inItem=$this->kclass->DB->fetchArray($inItems)){
									...
									$barcodeSelect.='<option value="'.$inItem['barcodeid'].'" selected>'.$inItem['barcode'].' ( 批次时间：'.date('Y-m-d',$inItem['arrivalTime']).' ) '.$qname.$inItem['store'].'</option>';
									$store = $inItem['store'];
								}
							}
							$barcodeSelect.='</select>';
							...
							if($this->kclass->DB->numRows()){
								...
								while($preInitem=$this->kclass->DB->fetchArray($preInitems)){
									$inItem=$this->kclass->DB->queryFirst("
										...
										LIMIT 0,1
									");
									$barcodeSelect.='<option value="'.$inItem['barcodeid'].'" selected>'.$inItem['barcode'].' ( 批次时间：'.date('Y-m-d',$inItem['arrivalTime']).' ) 库存：'.$inItem['store'].'</option>';
									$store =$inItem['store'];
								}
								$barcodeSelect.='</select>';
							}//
...
EOF;
	...
	function _getMaterial($b){
		//array('materialid'=>$val,'mid'=>$mid,'workcenterid'=>$this->kclass->input['workcenterid'])
     if($b==''){
		 return false;
	 }else{
      if($b['materialid']>=0){
	  ...
	  if($b['array']!='' AND is_array($b['array'])){
		  $array=$b['array'];
	  }
	  static $k=1;
		...
		for($i=1;$i<=count($array);$i++){
							$val=explode(',',$array[$i]);
							$materialid=$val[0];
							$materialitemid=$val[1];
								$items=$this->kclass->DB->queryFirst("
									SELECT ri.module,ri.mid,ri.mitemid,ri.materialid,ri.versionid,ri.materialitemid,ri.statusid,ri.reviseid,ri.dateline,ri.requirementid,
									m.materialno,m.title,m.standard,m.min,m.originid,
									mi.itemno,
									mv.title AS version,mv.`change`,mv.qualified,mv.planInbound,mv.planOutbound,
									materialorigin.title AS origin,
								...
								LEFT JOIN taskitem AS ti ON (ti.requirementitemid=ri.itemid)
								LEFT JOIN revise AS r ON (r.reviseid=ri.reviseid)
									WHERE ri.module='".$module."' AND ri.mid IN (".implode(',',$mid).")  AND ti.workcenterid='".$workcenterid."' AND m.materialid='".$materialid."'AND ri.materialitemid='".$materialitemid."'   AND ti.ifChooser=0 LIMIT 0,1
						");
					$quantity=0;
					$sql=$this->kclass->DB->query("
								SELECT ri.module,ri.mid,ri.materialid,ri.materialitemid,ri.quantity,ri.requirementid,ti.taskid,ti.workcenterid,ti.requirementitemid
								FROM requirementitem AS ri
								LEFT JOIN taskitem AS ti ON (ti.requirementitemid=ri.itemid)
									WHERE ri.module='".$module."' AND ri.mid IN (".implode(',',$mid).")  AND ti.workcenterid='".$workcenterid."' AND ri.materialid='".$materialid."'AND ri.materialitemid='".$materialitemid."'  AND ti.ifChooser=0
					");
					if($num=$this->kclass->DB->numRows()){
						while($sqls=$this->kclass->DB->fetchArray($sql)){
							$quantity+=$sqls['quantity'];
						}
					}
						...
						//判断是否存在子BOM
								...
								// 需求计划明细
				     $materialInfo=$this->kclass->getMaterial(array('materialid'=>$items['materialid'],'itemid'=>$items['materialitemid'])); // 获得物资信息
					 $child=$this->kclass->bomChildList(array('parentid'=>$items['materialid'],'materialitemid'=>$items['materialitemid'],'versionid'=>$items['versionid'],'quantity'=>$quantity,'loopNum'=>1,'ifGeneral'=>0));
								$itemList.='<tr class="'.$this->kclass->iif($this->kclass->rotate(),'odd','even').'  bold bgGreen">
												<td><input type="hidden" name="materialid['.$i.']" value="'.$items['materialid'].'"/>
												<input type="hidden" name="materialitemid['.$i.']" value="'.$items['materialitemid'].'">
												<input type="hidden" name="taskItemid['.$i.']" value="'.$items['taskid'].'">
												...
												<td>'.$materialInfo['standard'].'</td>
												<td>'.$this->kclass->iif($items['change']=='',$items['version'],'<a class="tip" href="#" title="( '.$items['change'].' )">'.$items['version'].$this->kclass->iif($items['itemno']!='',' ('.$items['itemno'].') ','').'</a>').'</td>
												<td><input type="hidden" name="itemQuantity['.$i.']" value="'.$quantity.'">'.$quantity.'</td>
												<td><input type="text" id="outTime'.$i.'" name="itemOutTime['.$i.']" size="10" value="'.date('Y-m-d',TIMENOW).'" ></td>
												<td><input type="text" name="remarkitem['.$i.']" value="'.$items['remark'].'"></td>
												<td align="center"><input type="checkbox" name="selectItemid['.$i.']" value="'.$items['materialid'].'"   ></td>
												</tr>';   
												$id.=$this->kclass->iif($i<=$b['num'],'#outTime'.$i.',','#outTime'.$i);
