...
EOF;
			...
			if($this->kclass->input['typeid']==15 AND $this->kclass->input['orderid']<=0){
				$order=$this->kclass->chooserOrder(array('name'=>'orderid','hasBlank'=>1,'topname'=>'请选择','condition'=>' AND o.ifOutbound=1 AND o.modifier=0','width'=>350,'route'=>'PO'));
				$body=<<<EOF
<form action="/s.php?module=outbound&action=add" method="get">
<input type="hidden" name="module" value="outbound">
<input type="hidden" name="action" value="add">
	...
	function out(){
		...
		if($outbound['typeid']==15){ //订单出库
			...
			if($outItem['quantity']==0){
				...
				$qname = "库存：";
			}elseif($outItem['quantity']>0){
				$orderid = $orderid-1;
				$sinboundItemid = $this->kclass->DB->queryFirst("SELECT sourceItemid,transferQuantity FROM ordertransfer WHERE targetMid='{$orderid}'");
				$inbounditem = $this->kclass->DB->queryFirst("SELECT barcodeid FROM inbounditem WHERE itemid='{$sinboundItemid['sourceItemid']}'");
				$barcodes = $this->kclass->DB->queryFirst("SELECT barcode,arrivalTime FROM barcode WHERE barcodeid='{$inbounditem['barcodeid']}'");
				$outquantity = $sinboundItemid['transferQuantity'];
				$barcodeid = $inbounditem['barcodeid'];
				$barcode = $barcodes['barcode'];
				$arrivalTime = $barcodes['arrivalTime'];
				$qname = "借调：";
			}
			...
			if($outItem['quantity']==0){
				...
				$orderid = $orderid-1;
				$sinboundItemid = $this->kclass->DB->queryFirst("SELECT sourceItemid,transferQuantity FROM ordertransfer WHERE targetMid='{$sampleid}'");
				$inbounditem = $this->kclass->DB->queryFirst("SELECT barcodeid FROM inbounditem WHERE itemid='{$sinboundItemid['sourceItemid']}'");
				$barcodes = $this->kclass->DB->queryFirst("SELECT barcode,arrivalTime FROM barcode WHERE barcodeid='{$inbounditem['barcodeid']}'");
				$outquantity = $sinboundItemid['transferQuantity'];
				$barcodeid = $inbounditem['barcodeid'];
...
EOF;
	...
	function doverify(){
		...
		$this->kclass->messager(array(
			...
			'url' => '/s.php?module=outbound&action=list',
			'sec' => 2
		));
	}
	// 入库申请单确认
