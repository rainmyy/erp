			...
			while($order=$this->kclass->DB->fetchArray($orders)){
				...
				if($order['killed']>0){
					...
					if($order['ifApprove']==1){
						$link=$this->kclass->iif($order['orderStatus']==0,' <a href="/s.php?module=porder&action=orderStatus&porderid='.$order['porderid'].'">完成/取消</a>','').' <a href="/p.php?action=porder&porderid='.$order['porderid'].'" target="_blank">印</a>';
			...
			for($m=1;$m<$j+1;$m++){
				if($this->kclass->input['itemSelect'][$m]>0){
					...
					$itemtr.='<tr class="'.$this->kclass->iif($this->kclass->rotate(), 'odd', 'even').' small">
						...
						'.$inputmaterialid.'
						'.$inputversionid.'
						<input type="hidden" name="itemUnitid['.$i.']" value="'.$item['unitid'].'">
						<td>'.$i.'</td>
						<td>'.$link.'</td>
						<td>'.$materialitem.'</td>
						<td>'.$item['version'].'</td>
						<td>'.$this->kclass->chooserBrand(array('name'=>'itemBrandid['.$i.']','hasBlank'=>1,'topname'=>'请选择','width'=>'100')).'</td>
						<td><input type="text" name="itemQuantity['.$i.']" size="5" value="'.$item['quantity'].'"> '.$item['unit'].'</td>
	...
	function insert(){
		...
		if($this->kclass->input['step']==2){
			...
			$this->kclass->DB->query("
				INSERT INTO `porder`
					(`orderno`,`noType`,`type`,`routeid`,`purchaseType`,`pinvoiceTypeid`,`supplierid`,`customerid`,`currencyid`,`deliverytermid`,`paymenttermid`,`paymentDate`,`taxRate`,`special`,`consignee`,`consigneeTel`,`spare`,`delivery`,`remark`,`created`,`creator`)
				VALUES
