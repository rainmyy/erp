...
EOF;
	...
	function view(){
		$requirement=$this->kclass->DB->queryFirst("
			SELECT r.requirementid,r.requirementno,r.statusid,r.modeid,r.orderChange,r.planner,r.remark,r.locked,r.killed,r.modified,r.created,r.typeid,
				mt.title AS type,
		...
		//
//		print_r($requirement['modeid'])
   if($requirement['typeid']==2){
		if($requirement['statusid']==1){//需求计划单未锁定
       ...
       if($requirement['modeid']==2){
              ...
              $addMrp='<span class="right"><a href="/s.php?module=task&action=add">新增生产任务</a></span>';
			}
		...
		if($this->kclass->input['mftType']=='normal'){
			...
			if($this->kclass->DB->numRows()){
				...
				while($order=$this->kclass->DB->fetchArray($orders)){
					//未投产订单（PO）
					$items=$this->kclass->DB->query("
						SELECT item.itemid,item.productid,item.productitemid,item.productAttributes,item.price,item.quantity,item.dateline,item.caption AS remark,
							p.title,
...
EOF;
	...
	function insert(){
		...
		if($this->kclass->input['requestMethod'] != 'post'){
			...
			if($this->kclass->input['typeid']==1){//订单需求
			...
			}elseif($this->kclass->input['typeid']==2){//库存需求
				$ifitem=0;
	...
	function dolock(){
		...
		if($this->kclass->input['statusid']==2){
			...
			$this->kclass->DB->query("
				...
				WHERE requirementid='".$this->kclass->input['requirementid']."'
			");
			$moduleid=$this->kclass->DB->query("SELECT module,mid FROM `requirementitem` WHERE requirementid='".$this->kclass->input['requirementid']."'");
			 if($num=$this->kclass->DB->numRows()){
				 $modulestr=array();
				 while($moduleids=$this->kclass->DB->fetchArray($moduleid)){
                    $modulearr[]=$moduleids['module'].','.$moduleids['mid'];
				 }
			 }
