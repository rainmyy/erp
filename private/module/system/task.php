		...
		if($this->kclass->DB->numRows()){
			...
			while($item=$this->kclass->DB->fetchArray($items)){
				...
				if(!empty($child['bom'])){
					...
					$bomtr.='<tr><td colspan="10" class="darkred">物资 [<a href="'.$materialInfo['url'].'">'.$materialInfo['no'].'</a>　'.$materialInfo['material'].'] BOM(子) 不存在！</td></tr>';
				}
				unset($child);
			}
		}else{
...
EOF;
			...
			if($this->kclass->DB->numRows()){
				...
				while($requirement=$this->kclass->DB->fetchArray($requirements)){
					if($requirement['typeid']==2){//库存需求
						$sql="SELECT mp.*,
							mv.title AS version,materialorigin.title AS origin
						FROM `mrp` AS mp
						...
						WHERE mp.killed=0  AND mp.ifChooser=0 AND mp.requirementid='".$requirement['requirementid']."'
						ORDER BY mp.ordering ASC";
                    
					}elseif($requirement['typeid']==1){//订单需求
						 $sql="SELECT ri.module,ri.mid,ri.itemid AS ritemid,ri.mitemid,ri.materialid,ri.versionid,ri.materialitemid,ri.statusid,ri.reviseid,ri.dateline AS requiredate ,ri.quantity AS netQuantity,ri.remark,
			...
			WHERE  ri.ifChooser=0  AND ri.requirementid='".$requirement['requirementid']."'
         ...
         $items=$this->kclass->DB->query($sql);
					...
					$itemtr.='<tbody><tr class="bold "><td colspan=9>需求计划单编号：<span class="middle bold">'.$requirementno.'</span>  　计划员：'.$requirement['creator'].'　'.$requirement['typeid'].'　计划时间：'.date('Y-m-d',$requirement['created']).'</td></tr><tr class="center even" nohover><td width="30">ID</td><td>物资编号</td><td>物资名称</td><td>物资规格</td><td>版本</td><td>需求数量</td><td>需求日期</td><td>备注</td><td>全选<input type="checkbox" name="itemAll" id="selectAll'.$n.'" onclick="dc.selectAll(\'selectAll'.$n.'\')" value="1"></td></tr>';
						while($item=$this->kclass->DB->fetchArray($items)){
							...
							$itemtr.='<tr class="'.$this->kclass->iif($this->kclass->rotate(),'odd','even').'">
								<td><input type="hidden" name="itemId['.$i.']" value="'.$this->kclass->iif($item['typeid']==1,$item['ritemid'],$item['mrpid']).'" /><input type="hidden" name="typeId['.$i.']" value="'.$requirement['typeid'].'">'.$k.'</td>
								...
								<td><input type="checkbox" class="select'.$n.'" name="itemSelect['.$i.']" value="'.$this->kclass->iif($requirement['typeid']==1,$item['ritemid'],$item['mrpid']).'"/></td>
							</tr>';
							$k++;$i++;
						}
							$n++;
...
EOF;
		...
		$this->kclass->page['onload'].='var dates=$(\''.$id.'\').datepicker({onSelect:function(selectedDate){instance=$(this).data(\'datepicker\'),date=$.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat,selectedDate,instance.settings);}});dc.tabhover();dc.tips();dc.selectAll();';
		...
		for($i=$k=1;$k<$count+1;$k++){
			if($this->kclass->input['itemSelect'][$k]!=''){
				if($this->kclass->input['typeId'][$k]==2){
				$item=$this->kclass->DB->queryFirst("
					SELECT mp.*,
						r.requirementid,r.requirementno,r.remark AS requirementremark,r.typeid,
						mv.title AS version
					FROM `mrp` AS mp
					LEFT JOIN `materialversion` AS mv ON (mv.versionid=mp.versionid)
				...
				");
					...
					$item=$this->kclass->DB->queryFirst("
					       SELECT ri.requirementid,ri.killed,ri.ifChooser,ri.itemid,ri.quantity AS netQuantity, ri.materialid, ri.remark,ri.materialitemid,ri.versionid,ri.dateline AS requiredate,
						               r.requirementid,r.requirementno,r.remark AS requirementmark,r.typeid,
									   mv.title AS version
						   FROM `requirementitem` AS ri
						   LEFT JOIN `requirement`  AS r ON(r.requirementid=ri.requirementid)
						   LEFT JOIN `materialversion` AS mv ON(mv.versionid=ri.versionid)
						   WHERE ri.killed=0 AND  ri.ifChooser=0 AND ri.itemid='".$this->kclass->input['itemSelect'][$k]."'
					");
				}
				$wc=$this->kclass->DB->queryFirst("
					SELECT wci.workcenterid,wc.title AS workcenter,wc.burthen
			...
			//	print_r($wc);
				$materialInfo=$this->kclass->getMaterial(array('materialid'=>$item['materialid'],'itemid'=>$item['materialitemid']));
				$workcenter=$this->kclass->chooserWorkcenter(array('id'=>'workcenterid'.$i.'','name'=>'workcenterid['.$i.']','width'=>150,'hasBlank'=>1,'selectedid'=>$wc['workcenterid']));
				$requirementno='<a href="/s.php?module=requirement&action=view&requirementid='.$item['requirementid'].'">'.$item['requirementno'].'</a>';
				$itemtr.='<tr class="'.$this->kclass->iif($this->kclass->rotate(), 'odd', 'even').' small">
					<input type="hidden" name="itemId['.$i.']" value="'.$this->kclass->iif($item['typeid']==1,$item['itemid'],$item['mrpid']).'" /><input type="hidden" name="typeId['.$i.']" value="'.$item['typeid'].'">
					<td>'.$i.'</td>
					...
					<td><input type="text" id="workdate'.$i.'" '.$this->kclass->iif($i==1,'onkeypress="inputAll(this.id)"','').'  name="itemWorkdate['.$i.']" size="15" /></td>
					<td><input type="text" id="finishdate'.$i.'" '.$this->kclass->iif($i==1,'onkeypress="inputAll(this.id)"','').' name="itemFinishdate['.$i.']" size="15" value="'.date('Y-m-d',$item['requiredate']).'" /></td>
					<td><input type="text" id="dispatchnumber'.$i.'" '. $this->kclass->iif($i==1,'onkeypress="inputAll(this.id)"','').' name="itemDispatchnumber['.$i.']" size="10" /> 人</td>
					<td><input type="text" id="workhours'.$i.'" '.$this->kclass->iif($i==1,'onkeypress="inputAll(this.id)"','').' name="itemWorkhours['.$i.']" size="15" /> 时</td>
					<td><input type="text" id="remark'.$i.'" name="itemRemark['.$i.']" value="'.$item['remark'].'" size="15" /></td>
				</tr>';
				//领料bom
				$bomtr.='<tr><td colspan="16" class="bold"><a href="'.$materialInfo['url'].'">'.$materialInfo['no'].'</a>　'.$materialInfo['material'].''.$materialInfo['standard'].'　 (版本：'.$item['version'].')</td></tr>';
...
EOF;
		...
		$this->kclass->page['onload'].='var dates=$(\''.$id.'\').datepicker({onSelect:function(selectedDate){instance=$(this).data(\'datepicker\'),date=$.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat,selectedDate,instance.settings);}});dc.tabhover();dc.inputAll();';
		$this->kclass->page['main']=$this->kclass->skin->dbody(array('title'=>$this->baseurl.=' - <a href="/s.php?module=task&action=list">生产任务单列表</a> - 新建生产任务单', 'right'=>'<a href="/s.php?module=task&action=list">返回列表</a>','body'=>$body));
	}
}
// insert task
	...
	if($this->kclass->input['addtype']=="hand"){//hand方式已作废
		...
		for($m=1;$m<$count+1;$m++){
			if($this->kclass->input['itemWorkdate'][$m]!=''){
				...
				if($this->kclass->input['typeId'][$m]==2){
				$item=$this->kclass->DB->queryFirst("
					SELECT *
					FROM `mrp`
				...
				");
                   ...
                   $item=$this->kclass->DB->queryFirst("
					...
					WHERE killed=0 AND ifChooser=0 AND itemid='".$this->kclass->input['itemId'][$m]."'
				");
				}
			/*修改前的增加工作中心
			*	$wc=$this->kclass->DB->queryFirst("
			...
			*	");
				*/
				...
				//$this->kclass->DB->query("UPDATE `requirementitem` SET ifChooser=1 WHERE mrpid='".$this->kclass->input['itemId'][$m]."'");
				}else{
					$this->kclass->DB->query("
					INSERT INTO taskitem (materialid,versionid,materialitemid,taskid,requirementid,requirementitemid,workcenterid,statusid,workdate,finishdate,dispatchnumber,workhours,quantity,remark,created,creator)
					VALUES ('".$item['materialid']."','".$item['versionid']."','".$item['materialitemid']."','".$taskid."','".$item['requirementid']."','".$this->kclass->input['itemId'][$m]."','".$this->kclass->iif($this->kclass->input['workcenterid'][$m]!='',$this->kclass->input['workcenterid'][$m],'0')."',1,'".$workdate."','".$finishdate."','".$this->kclass->input['itemDispatchnumber'][$m]."','".$this->kclass->input['itemWorkhours'][$m]."','".$item['quantity']."','".$this->kclass->input['itemRemark'][$m]."','".TIMENOW."','".$this->kclass->user['userid']."')
				");
...
EOF;
			...
			$typeid=$this->kclass->input['typeid'];
			if($typeid=="9" OR $typeid=="10" OR $typeid=="11" OR $typeid=="12"){
...
EOF;
			...
			$this->kclass->page['onload'].='dc.tabhover();dc.tips();';
			$this->kclass->page['main']=$this->kclass->skin->dbody(array('title'=>$this->baseurl.=' - 新建出库申请单', 'right'=>'<a href="/s.php?module=task&action=list">返回列表</a>','body'=>$body));
		}elseif($typeid=="19" OR $typeid=="20"){
			 if($typeid=="19"  ){
				  $title='订单领料';
				 $item='<tr class="odd">
                 <td title="'.$title.'" rowspan="3">请选择'.$title.'：<span class="red bold">*</span></td>';
				 for($i=1;$i<=3;$i++){
					 $orderorsample=$this->kclass->chooserOrder(array('name'=>'orderid['.$i.']','hasBlank'=>1,'topname'=>'请选择','condition'=>' AND o.ifOutbound=1','width'=>350,'route'=>'PO'));
					$item.=' <td>'.$orderorsample.'</td>
                   </tr>';
				 }
			 }else{
				 $title='样品单领料';
                 ...
                 <td title="'.$title.'" rowspan="3">请选择'.$title.'：<span class="red bold">*</span></td>';
				  for($i=1;$i<=3;$i++){
				$orderorsample=$sample=$this->kclass->chooserSample(array('name'=>'sampleid['.$i.']','hasBlank'=>1,'topname'=>'请选择','width'=>350));
				$item.=' <td>'.$orderorsample.'</td>
                   </tr>';
				  }
			 }
				 $body=<<<EOF
				 <form action="/s.php?module=outbound&action=add" name="task" method="post">
				 <input type="hidden" name="module" value="outbound"/>
				 <input type="hidden" name="action" value="add"/>
				 <input type="hidden" name="workcenterid" value="{$workcenterid}">
				 <input type="hidden" name="typeid" value="{$typeid}">
				 <table>
				 <thead>
				 <tr><th colspan="2">新建出库申请：{$title}</th></tr>
				 </thead>
				 <tbody>
                  {$item}
                    <tr class="odd" nohover>
...
function insertWorkcenter(){
	...
	$this->kclass->messager(array(
		'title' => '关联工作中心',
		'text' => '物资 [<b>'.$this->kclass->input['materialno'].'</b>] 修改工作中心成功!',
		'url' => '/s.php?module=task&action=view&taskid='.$this->kclass->input['taskid'],
		'sec' => 2
	));
}
