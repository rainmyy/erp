...
dc.addLine=function(m,eid){// 增加行数  m:module，eid:当前所在tr的id
	...
	}else if(m=='sampleProduct'){
		line='<tr class="" id="producttr'+i+'"><td>'+i+'</td><td><span id="tditemProductid'+i+'"></span><select id="itemProductid['+i+']" name="itemProductid['+i+']" style="width:500px;" onchange="dc.getProductEntitle(this.value,\'tditemProductid'+i+'\')">'+select[0].innerHTML+'</select></td><td><input type="text" name="itemProductCaption['+i+']" value="" size="60"></td><td><select id="itemProductBrandid['+i+']" name="itemProductBrandid['+i+']" style="width: 80px;">'+select[1].innerHTML+'</select></td><td><select id="itemProductPackingid['+i+']" name="itemProductPackingid['+i+']" style="width: 100px;">'+select[2].innerHTML+'</select></td><td><input type="text" name="itemProductDiscount['+i+']" value="100" size="3"></td><td><input type="text" name="itemProductQuantity['+i+']" size="3" onblur="dc.returnProductStr('+i+',this.value)"></td><td><select style="width: 80px;" name="itemProductUnitid['+i+']" id="itemProductUnitid['+i+']">'+select[3].innerHTML+'</select></td><td><input type="text" name="itemProductPrice['+i+']" size="5"></td></tr>';	
	}else if(m=='samplePart'){	
		line='<tr class="" id="parttr'+i+'"><td>'+i+'</td><td><select style="width:500px" name="itemPartsid['+i+']" id="itemPartsid['+i+']">'+select[0].innerHTML+'</select></td><td><input type="text" name="itemPartsCaption['+i+']" size="60"></td><td><select style="width:80px" name="itemPartsBrandid['+i+']" id="itemPartsBrandid['+i+']">'+select[1].innerHTML+'</select></td><td><select style="width:100px" name="itemPartsPackingid['+i+']" id="itemPartsPackingid['+i+']">'+select[2].innerHTML+'</select></td><td><input type="text" name="itemPartsDiscount['+i+']" value="100" size="3"></td><td><input type="text" name="itemPartsQuantity['+i+']" size="3" ></td><td><select style="width:80px" name="itemPartsUnitid['+i+']" id="itemPartsUnitid['+i+']">'+select[3].innerHTML+'</select></td><td><input type="text" name="itemPartsPrice['+i+']" size="5"></td></tr>';	
	}
...
dc.listItem={
	...
	page:function(p,m,w,d){//翻页 p:perpage
		...
		$.post(
			...
			{'start':this.s[m],'perpage':p,'amodule':m,'mmodule':w,'mid':d,'mtypeid':tid}
			)
		.fail(function() {
			$("#"+m+"page").html("加载失败！");
		})
...
dc.selectAll=function(id){
	if(typeof(id)!="undefined"){
		var Num= parseInt(id.match(/[0-9]/ig));
		$('#selectAll'+Num).click(function(){
		if($(this).attr('checked')){
			$('.select'+Num).attr('checked','checked');
		}else{
			$('.select'+Num).removeAttr('checked');
		}
	});
	}else{
...
dc.inputAll=function(id){
	var i;
	//var Num=parseInt($('#workdate').html());
	var Num=$("input[id^='workdate']").size();
	//var Num = $('#workdate').size();
	$('#workcenterid1').click(function(){
                for( i=1;i<=Num;i++){
					if($(this).attr('value')){
						$('#workcenterid'+i).attr('value',$(this).attr('value'));
					}else{
						$('#workcenterid'+i).removeAttr('value',$(this).attr('value'));
					}
				}
	});
	$('#workdate1').click(function(){
                for( i=1;i<=Num;i++){
					if($(this).attr('value')){
						$('#workdate'+i).attr('value',$(this).attr('value'));
					}else{
						$('#workdate'+i).removeAttr('value',$(this).attr('value'));
					}
				}
	});
	$('#finishdate1').click(function(){
                for( i=1;i<=Num;i++){
					if($(this).attr('value')){
						$('#finishdate'+i).attr('value',$(this).attr('value'));
					}else{
						$('#finishdate'+i).removeAttr('value',$(this).attr('value'));
					}
				}
	});
	$('#dispatchnumber1').click(function(){
                for( i=1;i<=Num;i++){
					if($(this).attr('value')){
						$('#dispatchnumber'+i).attr('value',$(this).attr('value'));
					}else{
						$('#dispatchnumber'+i).removeAttr('value',$(this).attr('value'));
					}
				}
	});
	$('#workhours1').click(function(){
                for( i=1;i<=Num;i++){
					if($(this).attr('value')){
						$('#workhours'+i).attr('value',$(this).attr('value'));
					}else{
						$('#workhours'+i).removeAttr('value',$(this).attr('value'));
					}
				}
	});
}
...
/**
 *新建发票时添加采购订单
 *@author Huang Jinlong 2015-01-21
 *@param none
 *@return none
 */
dc.addPorder = function(){
	$('#addSelect').click(function(){
		var target = $(this).parent().parent().prev();
		var select = target.html();
		var oname = target.find('select').attr('name');
		var Num = parseInt(oname.replace(/[^0-9]/ig,""))+1;
		$(this).parent().parent().before('<tr>'+select+'</tr>');
		$(this).parent().parent().prev().find('select').attr({name:'porderid['+Num+']',id:'porderid['+Num+']'});
	});
}
//
...
/**
 *物资盘点：点击添加区域插入输入框,输入完毕自动提交数据库并返回结果
 *@author	Huang Jinlong 2015-01-04
 *@param 	object args 包含参数jason格式数据
 *@return
 */
dc.inventory={
	firstIn:function(args){//初盘
		$('.first').live("click",function(){
			var invtype = 'f';
			var parent = $(this);
			$(this).html('<input type="text" name="firstInventory" size="6" />');
			$(this).children('input').focus();
			$(this).children('input').blur(function(){
				var mtype = args.mtype;
				var invid = args.inventoryid;
				var str = $(this).parent().siblings('.mid').children(0).attr('href');
				var mid = str.slice(46);
				var Num = $(this).val();
				if(Num==''){
					alert("盘点数量不能为空");
					return false;
				...
				var dif = Num-stock;			
				$.get('/s.php?module=ajax&action=addInventoryItem&mtype='+mtype+'&mid='+mid+'&stock='+stock+'&Num='+Num+'&invid='+invid+'&dif='+dif+'&invtype='+invtype,
					  function(data){
						//alert(data);
						parent.siblings('.final').text(Num);
						parent.siblings('.diff').text(dif);
						parent.replaceWith('<td align="center">'+Num+'</td>');
					  }
				);
			});
		});		
	},
	checkIn:function(args){//复盘
		$('.check').live("click",function(){
			...
			$(this).children('input').blur(function(e){
				...
				var stock = $(this).parent().prev().prev().text();
				var dif = Num-stock;
				//alert(dif);
				$.get('/s.php?module=ajax&action=addInventoryItem&mtype='+mtype+'&mid='+mid+'&Num='+Num+'&invid='+invid+'&dif='+dif+'&invtype='+invtype,
					  function(data){
						//alert(data);
						parent.siblings('.final').text(Num);
						parent.siblings('.diff').text(dif);
	...
	addRmark:function(args){//添加备注
		$('.remark').live("blur",function(){
			...
			var remark = $(this).val();
			var obj = $(this);
			//alert(remark);//test code			
			$.get('/s.php?module=ajax&action=addInventoryItem&mtype='+mtype+'&mid='+mid+'&invid='+invid+'&remark='+remark,
				  function(data){
					if(data==1){
						alert("请先录入盘点数据再填写备注信息！");
						obj.val('');
					}
				}
			);	
...
function stopMaterial(materialid,id){
	$("#stopMa"+id).remove();
	$.ajax({        
			  url: '/s.php?module=material&action=stopMaterial&materialid='+materialid,
